...
// internal/middleware/auth.go (SessionCookieName), and Ralto's identical copy
// in its own internal/middleware/core_session.go. Equiptra never issues or
...
// (not imported) because Core and Equiptra are separate Go modules/repos.
...
// CorePerson is the subset of Core's /api/whoami response the bridge needs —
// see simplified_suite_screens_v0_3.md §5 step 3.
...
// CoreAPIURL is exported so other server-to-server callers into Core (e.g.
// the Monday.com project-lookup proxy) share the same CORE_API_URL config
// this file already reads for the SSO bridge, rather than each reading the
...
// help here," so the caller should fall through to the classic session-cookie
// check and let it 401 as it always has. A non-nil error means Core couldn't
// be reached or returned something unparseable.
